...
der Klasse \texttt{ScriptedLoadableModule}. Die Klasse \texttt{ToothAnalyser}
...
Neben der Steuerungsaufgabe erstellt die Widget-Klasse auch die \ac{UI} aus der ui-Datei.
Da diese unter der Haube ein \ac{XML} Format aufweist, kann diese einfach in das
Modul hineingeladen werden. Der Quellcode aus \ref{lst:create_ui} zeigt dies.
...
gleichen Aufbau eingebaut wurden. Dies sorgt für ein einheitliches Vorgehen. Das
...
diese Kopplung herzustellen wird die Klasse mit dem Decorater \texttt{@parameterNodeWrapper}
...
umgebung gestartet wrid. Sollte es bei der Ausührung zu unerwarteten Fehlern kommen,
so bricht der Tooth Analyser den Vorgang mit einen Fehlermeldung ab. Um die Ausführung
in der entsprechenden Logik-Klasse genauer zu betrachten, sei hier auf den
...
sich mit den verschiedenen Tests un deren Auswertung. Begonnen wird mit einer
...
ausgegangen werden. Zu Beachten ist an dieser Stelle, das eine Filterung unter
Umständen einige Zeit in Anspruch nehmen kann. Es sei gesagt, dass diese bei
einer Testausführung zu berücksichtigen ist. Ein gute Option dem entgegen zu wirken
ist es, das bereits gefilterte Bild mit in den Testdaten zur Verfügung zu
stellen. Die konkrete Implementierung eines solchen Tests liefert der Quellcode \ref{lst:test_smooth_image}.
...
Abschließend lässt sich über die Softwaretests sagen, das einige Testfälle
abgedeckt wruden. Jedoch wurden nicht alle Methoden und Funktionen getestet.
...
wurde und keine medialen Flächen erfordert. Unter Berücksichtigung von Abbildung
\ref{fig:laufzeit} Kann dann die Zeit für die Filterund und die medialen Flächen
abgezogen werden. So kommt es, dass das Verfahren nur noch 16 Prozent der
...
\ref{fig_laufzeit_batch}. Ist die Zeit eines einzelnen Bildes bekannt, so lässt sich
...
In diesem Fall würde die Bearbeitung von zehn Bilder etwa 170 Minuten beanspruchen.
Dies entspricht zwei Stunden und 50 Minuten.
...
einer micro \ac{CT}-Aufnahme ein 3D Modell des Zahnes erstellen. Man spricht hier
