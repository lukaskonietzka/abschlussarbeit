...
Evaluation der Ergebnisse begonnen werden. Ein wichter Teil sind die Tesergebnisse
% ---------------------------------------------------------------------------------------

\section{Evaluierung}
\label{subsec:evaluierung} Neben der Implementierung der Features wurden auch diverse Tests
durchgeführt. Hierzu wurden Softwaretests, Usertests und Messungen durchgeführt. Dieses
Kapitel beschäftigt sich mit der Testauswertung der verschiedenen Testfälle. Begonnen wird
mit einer Analyse der Softwaretests, gefolgt von Performancemessungen. Abschließend werden
die Anwendungsfälle des Tooth Analyser näher beleuchtet und auf die Limitierungen der
Anwendung eingegangen.

\subsection{Softwaretests}
\label{subsec:softwaretests} Betrachtet man die Dokumentation von Slicer genauer, so fällt
auf, dass dies eine recht starre Stuktur für die Implementiernug von Tests vorgiebt. Dabei
ist jedoch nicht festgelegt, welche Art von Tests verwendet werden soll. Im Rahmen des
Tooth Analyser wurden hier ausschließlich Unittest implementiert, welche die einzelnen
Einheiten und Funktionen im Tooth Analyser abdecken. Ein grober Testaufbau
sei hier gezeigt.
...
- hier die sample Date erwähnen, die regestrerit wrude

- die konkrete methode zeigen für das registrerein und laden

- dann sagen dass das filtern über die standardabwiechung getetst werden kann

- wenn ein bild rauscht, dann ist die Streuung sehr hoch. nach der fitlerung sollte
sie kleiner seina ls zuvor.



Prinzipiell sei gesagt, dass der Fokus der Evaluierung auf den Funktions- und Usertest lag.
% ---------------------------------------------------------------------------------------
