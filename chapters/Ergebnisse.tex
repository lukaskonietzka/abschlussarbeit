...
Teilaufgaben aus \ref{sec_zerlegung_in_teilprobleme}. Zulestzt soll die Performance
...
kann unter Berücksichtigung eine Aussage bezogen auf die Forschungsfrage gestellt
werden.
% ---------------------------------------------------------------------------------------

\section{Tooth Analyser}
\label{sec:tooth_analyser} Im Rahmen dieser vorliegenden Arbeit ist eine 3D Slicer
Extension entstanden, die den Namen Tooth Analyser trägt und für die Forschung
...
Treten während der Bedienung der Software noch weitere Fehler auf, so verfügt der
Tooth Anaylser auch über ein ausführliche Dokumentation. Diese ist im
öffentlichen Repository der Erweiterung einsehbar und in vier kleine
Dokumentationen gegliedert \citep[vgl.][]{procida2017}. Der Link zur
...
Alle drei erben von der Klasse \texttt{ScriptedLoadableModule}. Die Klasse
\texttt{ToothAnalyser} bildet hierbei den Abschnitt \textit{Help and
...
ui-Datei. Da diese unter der Haube ein \ac{XML} Format aufweist, kann diese einfach
...
sichergestellt, dass alle Algorithmen nach dem gleichen Aufbau eingebaut wurden.
...
Um die Architektur des Tooth Analyser noch genauer nachvollziehen zu können, soll
...
Funktionen in unterschiedlichen Klassen zu finden sind.
...
herzustellen wird die Klasse mit dem Decorater \texttt{@parameterNodeWrapper} versehen.
...
gestartet wrid. Sollte es bei der Ausührung zu unerwarteten Fehlern kommen, so bricht
der Tooth Analyser den Vorgang mit einen Fehlermeldung ab. Um die Ausführung in
der entsprechenden Logik-Klasse genauer zu betrachten, sei hier auf den nachfolgenden
Quellcode verwiesen.
...
Kapitel beschäftigt sich mit den verschiedenen Tests un deren Auswertung. Begonnen
...
Betrachtet man die konkrete Testmethode \texttt{testIsSmoothed()} genauer, so fällt
die Methode \texttt{getSampleDataAsITK()} auf, die hier kurz thematisiert werden
soll. Viele der geschriebenen Methoden und Funktionen benötigen für einen guten
Test ein konkretes Bild. Hierfür stellt der Tooth Analyser Beispielbilder zur Verfügung,
mit denen die Tests ausgeführt werden können. Da diese Bilder mit ca. 500 \ac{MB}
eine ausgeprägte Größe haben, wurden diese in einem separaten Repository
...
kann so auch Benutzern Beispielbilder bereitgestellt werden, um erste Erfahrungen
...
ausgegangen werden. Zu Beachten ist an dieser Stelle, das eine Filterung unter Umständen
einige Zeit in Anspruch nehmen kann. Es sei gesagt, dass diese bei einer Testausführung
zu berücksichtigen ist. Ein gute Option dem entgegen zu wirken ist es, das bereits
gefilterte Bild mit in den Testdaten zur Verfügung zu stellen. Die konkrete
Implementierung eines solchen Tests liefert der Quellcode
\ref{lst:test_smooth_image}.
...
Abschließend lässt sich über die Softwaretests sagen, das einige Testfälle abgedeckt
wruden. Jedoch wurden nicht alle Methoden und Funktionen getestet. Viele bilden eine
...
von 3D Mikro-\ac{CT}-Aufnahmen sehr stark vom Type des Bildes abhängt. So kommt es
...
Abbildung \ref{fig:laufzeit} Kann dann die Zeit für die Filterund und die medialen
...
im Diagramm \ref{fig_laufzeit_batch}. Ist die Zeit eines einzelnen Bildes bekannt,
...
mittels einer micro \ac{CT}-Aufnahme ein 3D Modell des Zahnes erstellen. Man
...
\begin{minipage}{0.45\textwidth}
	...
	\captionof{figure}{Rekonstruktion eines Zahns aus einer \ac{CT}-Aufnahme mittels der Erweiterung Tooth Analyser}
	\label{fig:3d_view}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
	...
	\captionof{figure}{Segmentbetrachtung eines rekonstruierten Zahns auf Basis einer \ac{CT}-Aufnahme}
