...
\begin{figure}[h]
	\centering
	\fbox{\includegraphics[width=0.5\textwidth]{img/data_module.png}}
...
\section{Tooth Analyser Beispieldaten}
...
das Modul aufruft. Neben dem Konstruktoraufruf der Elternklasse, werden auch die Parameter
und die Logik-Schnittstelle initialisiert. So ergibt sich die Situation, dass die
\ac{UI} die aktuellen Parametereinstellungen abfragt und sie an die Logik
weiterleitet.
...
Standardimplementierung haben und direkt in der Schnittstelle implementiert werden. Die
...
Programmes, welcher als \textit{Parameter node} bekannt ist. Der \textit{Parameter node}
ist laut \citet{slicer2024} eine leichte Variante eines \ac{MRML}-Knoten um Parametereinstellungen
zu speichern. Durch das Zusammenspiel zwischen \ac{UI} und \textit{Parameter node}
...
Um diese Kopplung herzustellen, wird die Klasse mit dem Decorater \texttt{@parameterNodeWrapper}
versehen. Die Klasse \texttt{ToothAnalyserWidget} hat vollen Zugriff auf die Parameter-Klasse
...
liegt. Die Methode  aus \ref{lst:on_apply} sorgt außerdem dafür, dass der Algorithmus in einer sicheren Umgebung
