...
Einleitung klar wurde, handelt es sich bei den Mikro-CT-Bilder um Zahnbilder. Um
zu verstehen, wie eine CT-Aufnahme technisch segmentiert und damit zerlegt werden
kann, muss zunächst die Zahnstruktur selbst verstanden werden.
...
\begin{minipage}{0.40\textwidth}
	...
	auch Zahnbein genannt den Großteil eines Zahnes einnimmt. Im Bereich der
	Zahnkrone wird das Dentin von Zahnschmelz überzogen. Der Zahnschmelz ragt in die
	Mundhöhle und ist nach \citet[Seite 41]{lehmann2012Zahnheilkunde} das härteste
	Material im menschlichen Körper. In der Mitte des Zahnes befindet sich
	Weichgewebe, das als Pulpa bezeichnet wird vgl. \citep[Seite ]{lehmann2012Zahnheilkunde}.
\end{minipage}
\hfill
...
Für die Bearbeitung von Mikro-CT-Aufnahmen sind die Bereiche Schmelz, Dentin und
Pulpa von besonderer Bedeutung. Betrachtet man eine CT wie es zu Beginn in der Abbildung
\ref{fig:ct_aufnahme_eines_zahns} gezeigt wurde, so bilden diese drei
Gewebearten die unterschiedlichen Grauwerte in einem CT-Bild.
...
die einzelnen Zahnsubstanzen auf einer CT-Aufnahme lokalisieren.
...
Bilddaten. Dieser Abschnitt erläutert federführend die Technologie der Mikro-CT-Aufnahmen
und deren Erstellung. Diese sind für eine medizinischen Einsatz besonders interessant.
Des Weiteren erfolgt eine Einführung in die Speicherung und Komprimierung von CT-Aufnahmen.
Diese sorgt dafür, dass die digitalen Bilddaten deutlich handlicher werden.
% ---------------------------------------------------------------------------------------
...
mehr wegzudenken. Ein Mikro-CT-Bild ist laut \citet[Abstract]{baird2017} ein
...
ist verschiedene Materialien in einer CT-Aufnahme zu unterscheiden \citep[vgl.][]{nib2024}.

Für eine Mikro-CT-Aufnahme bedarf es spezieller Technik. Es gibt unterschiedliche
Firmen, welche die unterschiedlichsten Modelle anbieten. Im Falle der Zahnklinik
an der \ac{LMU} handelt es sich um ein Mikro-CT 40 der Firma \citet{scanco2024}.
...
abgelegt wird. Wie das nächste Kapitel beschreiben wird, ist der Speicherumfang
den solch ein Bild benötigt sehr groß, jedoch auch sehr detaliert. Um ein
einfacheres Arbeiten mit den CT-Bildern zu ermöglichen, kann eine simple Technik
angewendet werden, die hier beschrieben werden soll.
% ---------------------------------------------------------------------------------------

\subsection{Datenformate}
\label{subsec:datensätze} Die rohen Datensätze, welche direkt aus dem Mikro-CT-Gerät
...
Durch die Gleichungen in \ref{equ:größe_bestimmen} wird klar, dass eine CT-Aufnahme
des Typs \ac{ISQ} direkt nach seiner Aufnahme über einen \ac{GB} groß ist. Laut \citet{poliklinikLMU}
...
genauer, so fällt auf, dass es noch weitere Typen gibt, die durch eine geringere
Länge $k$ deutlich weniger Speicher benötigen. Durch Anwendung simpler Statistik
lässt sich herauslesen, dass die $2^{4}$ Byte je Element nicht ausgenutzt werden.
...
noise} undn\textit{roundoff noise}.
...
auch Filter gennant, hat die Aufgabe dieses Rauschen so gut wie möglich zu reduzieren.
...
interpretiert werden kann und die Nachbarschaft eines Pixels abbildet. Abbildung
\ref{fig:lokaler_operator_maske} zeigt eine solche Maske und soll das Verfahren so
verdeutlichen. Der grau hinterlegte Mittelpunkt - $P = (0/0)$ - ist das aktuell
betrachtete Pixel. die Felder um die Mitte herum die Nachbarn. Es fällt jedoch
auf, dass durch dieses Schema nicht jede mögliche Ausprägung einer Maske in Frage
kommt. Um einen Mittelpunkt und damit einen aktuellen Pixel betrachten zu können,
bedarf es eines ungeraden Grades für $M$. Diese Eingränzung lässt sich in
Gleichung \ref{equ:lokaler_operator} generisch fassen.
...
angewendet werden. \citet[Seite 54 - 55]{handels2000} unterscheidet hier in Glättungsfilter,
...
zu erlangen, sei an dieser Stelle auf \citet[Seite 54 - 55]{handels2000} verwiesen.
...
Orginalbild zu berechnen. Dies kann im Falle eines CTs der Zahnklinik an der \ac{LMU}
die hervorgehobene Darstellung der Zahnsubstanzen Schmelz und Dentin sein.
\citep[vgl.][Seite 359]{lehmann2013bildverarbeitung}. Konkret teilt ein Segmentierungsverfahren
also ein Bild in Teilbereiche auf. Dabei sind die Teilbereiche in sich
bemerkenswert homogen. \citet[Seite 1]{ramesh2021} beschreiben, dass der Prozess
...
als zentrales Problem darstellt. \citet[Seite 95]{handels2000} und \citet[Seite 360]{lehmann2013bildverarbeitung}
beschreiben beide, dass die Bildsegmentierung eines CTs für eine gute und
eindeutige Ärzliche Diagnose nicht mehr wegzudenken ist. Warum dem so ist,
...
(Interpretation 1 und Interpretation 2). \citet[Seite 360]{lehmann2013bildverarbeitung}
...
Verfahren, welche eine gute Schwellwertwahl gewährleistet, ohne dass zu viele Informationen
...
den Schwellwert $t$ durch ein statistisches Gütekriterieum. Hierzu bedient sich das
...
Mikro-CT-Bilder des Zahnklinikums an der \ac{LMU} in München entwickelt.
...
\citet{hoffmann2020} beschreibt, dass dieses Verfahren bis zu einem gewissen Fortschritt
...
Die Kernanwendung von 3D Slicer folgt einem Softwarepattern, dass sich \ac{MVC}
...
\cite[Seite 1332]{fedorov2012slicer} mit der Abbildung \ref{fig:3d_slicer_architektur}.
...
in der Kernanwendung. Das indentische Pattern spiegelt sich auch in den
einzelnen Modulen von Slicer wieder. So wird sichergestellt, dass ein
...
und zu speichen, die für einen klinischen Zweck Einsatz finden \citep[vgl.][]{slicer2024}.
Laut der \citet{slicer2024} wurde die MRML-Datenstruktur völlig unabhängig von der
Slicer Kernanwedung entwickelt. Dies ermöglicht ein portieren der Datenstruktur
auf andere Softwareapplikationen. Da Slicer die einzig große Plattform ist, die
diese Datenstruktur nutzt, wird der Quellcode für MRML im Reporsitory von 3D Slicher
gewartet und weiterentwickelt, so die \citet{slicer2024}. Durch den Artikel von
\citet[Seite 1327]{fedorov2012slicer} wird klar, dass MRML mehr ist also nur eine
Datenstruktur. Sie beschreiben MRML als Szenenorganisator von Bilder, Anotationen,
...
MRML zeichnet sich vorallem dadurch aus, dass es eine Vielzahl an Dateiformaten akzeptiert.
...
sind hier bei \ac{VTK} und \ac{ITK} \citep[vgl.][]{vtk2024}, \citep[vgl.][]{itk2024}.
Dies beiden Tools sind echte Rießen in ihrere Branche. \ac{MRML} nutzt diese um einige
Dateiformate zu lessen und zu schreiben.
...
\label{subsec:benutzerschnitstelle} Für das erstellen eines \ac{UI}, das für eine
Slicer Extention nötig ist, nutzt 3D Slicer den Qt-Designer \citep[vgl.][]{qt2024}.
